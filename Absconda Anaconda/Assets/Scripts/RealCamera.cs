...
public class RealCamera : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        ...
        transform.Rotate(new Vector3(Input.GetAxis("Mouse Y"), 0, 0) * -camYSensitivity);
        transform.Translate(new Vector3(0, Input.GetAxis("Mouse Y") * -0.1f));
        //transform.position = new Vector3(Mathf.Lerp(transform.position.x, target.transform.position.x, lerpSpeed * Time.deltaTime), Mathf.Lerp(transform.position.y, target.transform.position.y, lerpSpeed * Time.deltaTime), Mathf.Lerp(transform.position.z, target.transform.position.z, lerpSpeed * Time.deltaTime));
